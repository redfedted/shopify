...
const FlowerCard = ({ flower, onClick }) => {
  return React.createElement('button', { 
    onClick: onClick, 
    className: "flex flex-col items-center justify-start p-4 w-72 h-64 select-none transition-transform duration-300 ease-in-out hover:scale-105 group text-left" 
  },
    React.createElement('div', { className: "w-48 h-48 flex items-center justify-center" },
    ...
    React.createElement('p', { className: "mt-4 italic text-xl text-stone-800" }, flower.name)
  );
};

const ScrollableGridGallery = ({ onFlowerClick }) => {
  ...
  const handleCardClick = (e, flower) => {
    if (didMove.current) {
  ...
  return React.createElement('div', { 
    ...
    galleryFlowers.map((flower, index) => 
      React.createElement('div', { 
        ...
        onClick: (e) => handleCardClick(e, flower) 
      },
        React.createElement(FlowerCard, { 
          flower: flower, 
          onClick: () => { /* Clicks are handled by parent div */ } 
        })
      )
    )
  );
